...
def get_product_data_from_browser(page, live_object_id):
    ...
    try:
        ...
        # 提取表格字段名（从 .ant-table-header 中所有的 th 的 textContent()）
        try:
            # 先尝试从 .ant-table-header 中查找
            header_ths = page.query_selector_all('.ant-table-header th')
            field_names = []
            ...
            if header_ths:
                for th in header_ths:
                    try:
                        ...
                        field_names.append('')  # 添加空字段名，保持索引一致
                        continue
